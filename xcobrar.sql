...
CREATE PROCEDURE [dbo].[OBJREPORTES_CONSOLIDADO_SALDOSXCOBRAR_ORIGEN]
...
AS
    BEGIN
        ...
        DECLARE
    		@FECHAFIN CHAR(8) = FORMAT(GETDATE(),'yyyyMMdd'),
            @IDCAMPANA CHAR(4) = '',
	        @IDPROVEEDOR CHAR(11)=''
        ...
        ELSE
			...
			print'pruebA'
                ...
                FROM    (SELECT C.IDCLIEPROV,
                                ...
                                C.FECHA,
                                C.VENCIMIENTO,
								--CAST(CONVERT(DATETIME, @FECHAFIN, 112) - ISNULL(C.VENCIMIENTO,C.FECHA) AS NUMERIC(10, 0)) AS VENCIDO,
                               datediff(DAY,CONVERT(DATETIME, @FECHAFIN, 112),ISNULL(C.VENCIMIENTO,C.fecha)) AS VENCIDO,
                                MO.NOMBRE_CORTO AS MONEDA,
                                ((C.IMPORTE + ISNULL(TF.IMP_FISE, 0)) * D.FACTOR) AS IMPORTEMOF,
                                CAST(0.00 AS NUMERIC(17, 2)) AS PAGOSMOF,
                                SUM(M.IMPORTEMOF * M.FACTOR) AS SALDOMOF,
